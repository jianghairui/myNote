 ...
 * 3，一个用户可以属于多个用户组(mp_auth_group_access表 定义了用户所属用户组)。我们需要设置每个用户组拥有哪些规则(mp_auth_group 定义了用户组权限)
 * 4，支持规则表达式。
 *      在mp_auth_rule 表中定义一条规则时，如果type为1， condition字段就可以定义规则表达式。 如定义{score}>5  and {score}<100
