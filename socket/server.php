...
//        var_dump($this->sockets);
    ...
    public function WebSocket($address,$port){ //服务器监听
        ...
        socket_set_option($server, SOL_SOCKET, SO_REUSEADDR, 1);//boolean true
        //绑定 address 到 socket。 该操作必须是在使用 socket_connect() 或者 socket_listen() 建立一个连接之前。
        socket_bind($server, $address, $port);//boolean true
        //在socket套接字已创建使用socket_create()定界与socket_bind()名称，它可以告诉听套接字传入的连接.
        socket_listen($server);//boolean true
    ...
    public function uncode($str,$key){ //返编译
        $mask = array();
        $data = '';
